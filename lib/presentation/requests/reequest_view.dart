                  ...
                  child: Padding(
                    ...
                    child: RefreshIndicator(
                      onRefresh: () async {
                        ...
                        bloc.add(GetPropertiesRequestEvent());

                        // Wait briefly to allow PropertyRequestsBloc to finish
                        await Future.delayed(const Duration(milliseconds: 300));

                        final state = bloc.state;
                        if (state is PropertyRequestsSuccess) {
                          final ids =
                              state.getRequestsFromLawyerResponse.data!
                                  .map((e) => e.propertyForSaleId!)
                                  .toList();
                          cubit.refreshAllNegotiations(ids);
                        }
                      },
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Padding(
                            ...
                            child: RequestsItem(
                              key: ValueKey(
                                "${propertyItem.requestFromLawyerId}_${DateTime.now().millisecondsSinceEpoch}",
                              ),
                              item: propertyItem,
                              isHistory: false,
                            ),
