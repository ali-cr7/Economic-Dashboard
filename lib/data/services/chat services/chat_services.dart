...
//   ) async {
//     try {
//       print(' the sending endboin${APIConfig.getChatEndpoint(APIConfig.chatRoomsEndpoint)}');
//       final response = await _apiService.post(
//         endpoint: APIConfig.getChatEndpoint(APIConfig.chatRoomsEndpoint),
//         data: {'currentUserId': currentUserId, 'otherUserId': otherUserId},
//       );

//       if (response.servicesResponse == ServicesResponseStatues.success) {
//         final roomId = response.fullBody?['roomId'];
//         joinChatRoom(roomId);
//         return roomId;
//       } else {
...
//         endpoint: APIConfig.getChatEndpoint(
//           '${APIConfig.chatRoomsEndpoint}/$userId',
//         ),
//       );

//       if (response.servicesResponse == ServicesResponseStatues.success) {
//         final List<dynamic> data = response.fullBody?['rooms'] ?? [];
//         return data.cast<Map<String, dynamic>>();
//       } else {
//         throw Exception(response.response);
//       }
...
//     try {
//       final response = await _apiService.post(
//         endpoint: APIConfig.getChatEndpoint(APIConfig.chatMessagesEndpoint),
//         data: {'chatRoomId': chatRoomId, 'senderId': senderId, 'text': text},
//       );
...
//       final response = await _apiService.get(
//         endpoint: APIConfig.getChatEndpoint(
//           '${APIConfig.chatGetMessagesEndpoint}/$chatRoomId',
//         ),
//         query: {
//           'limit': limit.toString(),
//           if (lastMessageTimestamp != null)
...
//       if (response.servicesResponse == ServicesResponseStatues.success) {
//         final List<dynamic> messagesData = response.fullBody?['messages'] ?? [];
//         return messagesData.map((m) => Message.fromJson(m)).toList();
//       } else {
//         throw Exception(response.response);
//       }
//     } catch (e) {
//       throw Exception('Error getting historical messages: $e');
//     }
//   }
...
//     final socket = IO.io(APIConfig.chatServiceUrl, <String, dynamic>{
//       'transports': ['websocket', 'polling'],
//       'autoConnect': true,
//       'reconnection': true,
//       'reconnectionAttempts': 5,
